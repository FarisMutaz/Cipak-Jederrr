...
export default function StokOpnamePage() {
  ...
  // Mutation: Bulk Delete operational stocks
  const bulkDeleteMutation = useMutation({
    mutationFn: async (stockIds: string[]) => {
      const res = await fetch("/api/stok-opname/bulk", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || "Gagal menghapus barang terpilih");
      }
      return res.json();
    },
    onSuccess: (data) => {
      triggerAlert("success", data.message || "Barang terpilih berhasil dihapus!");
      setSelectedStockIds([]);
      queryClient.invalidateQueries({ queryKey: ["stok-opname-list"] });
      queryClient.invalidateQueries({ queryKey: ["stok-opname-movements"] });
  ...
  const handleBulkDelete = async () => {
    const ok = await confirm({
      ...
      message: `Apakah Anda yakin ingin menghapus ${selectedStockIds.length} barang perlengkapan terpilih? Seluruh data stok dan riwayat mutasi terkait barang tersebut akan dihapus dari pandangan.`,
      confirmText: "Ya, Hapus Semua",
      variant: "danger",
    });
    if (ok) {
      bulkDeleteMutation.mutate(selectedStockIds);
    }
  };
  ...
  const allShownStockIds = filteredStocks.map((s: any) => s.id);
  const isAllSelected = allShownStockIds.length > 0 && allShownStockIds.every((id: string) => selectedStockIds.includes(id));
  const isSomeSelected = selectedStockIds.length > 0 && !isAllSelected;

  const handleSelectAll = () => {
    if (isAllSelected) {
      setSelectedStockIds([]);
    } else {
      setSelectedStockIds(allShownStockIds);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in relative pb-10">
      ...
      <AnimatePresence>
        {isOwnerOrDev && selectedStockIds.length > 0 && (
          <motion.div
